...
{#if current}
  <div class="overlay" role="presentation" on:click|self={close}>
    <div class:ffmpeg={current.kind === 'ffmpeg-missing'} class="dialog" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <header><div class="small-icon" aria-hidden="true">{current.kind === 'ffmpeg-missing' ? '△' : '×'}</div><h2 id="modal-title">{current.title}</h2><button class="close" type="button" on:click={close} aria-label="Close">×</button></header>
...
<style>
  ...
  header{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:12px}.small-icon{width:30px;height:30px;display:grid;place-items:center;color:var(--status-danger);border:1.5px solid currentColor;border-radius:50%;font-weight:700}.ffmpeg .small-icon{color:var(--status-warning)}.close{color:var(--text-muted);font-size:20px}.dialog pre{max-height:160px;overflow:auto;white-space:pre-wrap;padding:12px;background:var(--surface-sunken);border-radius:6px;color:var(--text-secondary)}
</style>
